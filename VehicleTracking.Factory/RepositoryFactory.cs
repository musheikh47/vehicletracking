...
namespace VehicleTracking.Factory
{
    public static class RepositoryFactory
    {
        ...
        // For unit testing we can return mock repository from this point. In our system only this project will be aware that we are using EFModel. All the projects will be using interfaces.
        public static T GetRepository<T>() where T : class
        {
            var repository =  default(T);
            var type = typeof(T);
            if (type == typeof(IVehicleRepository))
